...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_tournament'])) {
        ...
        // Handle picture upload
        $picture = '../assets/images/Esports Tournament.jpg'; // Default image
        if (isset($_FILES['picture']) && $_FILES['picture']['error'] === 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO tournaments (tournament_name, game_type, entry_fee, prize_money, slots, picture, status, tournament_date, registration_deadline) 
                               VALUES (?, ?, ?, ?, ?, ?, 'upcoming', ?, ?)");
        $stmt->bind_param("ssddisss", $tournament_name, $game_type, $entry_fee, $prize_money, $slots, $picture, $tournament_date, $registration_deadline);
    ...
    if (isset($_POST['generate_fixtures'])) {
        ...
        $stmt = $conn->prepare("SELECT t.*, COUNT(r.id) as registered_teams 
                              FROM tournaments t 
                              JOIN team_registrations r ON t.id = r.tournament_id 
                              WHERE t.id = ? AND r.payment_status = 'confirmed' 
                              GROUP BY t.id");
        $stmt->bind_param("i", $tournament_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($tournament && $tournament['registered_teams'] == $tournament['slots']) {
            ...
            try {
                ...
                for ($i = 0; $i < count($teams); $i += 2) {
                    if ($i + 1 < count($teams)) {
                        $stmt = $conn->prepare("INSERT INTO fixtures (tournament_id, team1_id, team2_id, match_date, round, match_number, status) 
                                               VALUES (?, ?, ?, ?, ?, ?, 'pending')");
                        $stmt->bind_param("iiissi", $tournament_id, $teams[$i]['registration_id'], $teams[$i+1]['registration_id'], $match_date, $round, $match_number);
                        $stmt->execute();
                        $match_number++;
                        $match_date = date('Y-m-d H:i:s', strtotime($match_date . ' +3 hours'));
                    }
                }
    ...
    if (isset($_POST['edit_tournament'])) {
        ...
        $stmt = $conn->prepare("UPDATE tournaments SET tournament_name = ?, game_type = ?, entry_fee = ?, prize_money = ?, slots = ?, picture = ?, tournament_date = ?, registration_deadline = ? WHERE id = ?");
        $stmt->bind_param("ssddisssi", $tournament_name, $game_type, $entry_fee, $prize_money, $slots, $picture, $tournament_date, $registration_deadline, $tournament_id);
...
<head>
    ...
    <link rel="stylesheet" href="/Esports_Tournament/assets/css/admin.css">
</head>
<body>
    <div class="container">
        <!-- Navigation -->
        <div class="nav-top">
            ...
            <div class="nav-buttons">
                <div class="system-buttons">
                    <a href="/Esports_Tournament/index.php" class="btn btn-secondary"><i class="fas fa-home"></i> Home</a>
        ...
        <div class="content-sections">
            ...
            <div id="addTournamentSection" class="form-container" style="display: none;">
                <h3><i class="fas fa-trophy"></i> Add New Tournament</h3>
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="game_id" name="game_id" required>
                            <?php
                            $games_list = $conn->query("SELECT id, game_name FROM games ORDER BY game_name");
                            while($game = $games_list->fetch_assoc()):
                            ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control" id="prize_money" name="prize_money" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="picture">Tournament Banner</label>
                        <input type="file" class="form-control-file" id="picture" name="picture">
                    </div>
                ...
                <form method="POST" class="tournament-form">
                    <div class="form-group">
                        ...
                        <select name="tournament_id" class="form-control" required>
                            ...
                            if (($tournaments_result && $tournaments_result->num_rows == 0) && ($ongoing_result && $ongoing_result->num_rows == 0)) {
            ...
            <div id="viewTournamentsSection" class="form-container" style="display: none;">
                ...
                <div id="editTournamentForm" style="display: none;" class="edit-form-container">
                    ...
                    <form method="POST" enctype="multipart/form-data" class="edit-form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="edit_tournament_name" name="tournament_name" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_game_id">Game</label>
                            <select class="form-control" id="edit_game_id" name="game_id" required>
                                <?php
                                $games_list = $conn->query("SELECT id, game_name FROM games ORDER BY game_name");
                                while($game = $games_list->fetch_assoc()):
                                ?>
                        ...
                        <div class="form-group">
                            <label for="edit_entry_fee">Entry Fee</label>
                            <input type="number" step="0.01" class="form-control" id="edit_entry_fee" name="entry_fee" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_prize_money">Prize Money</label>
                            <input type="number" step="0.01" class="form-control" id="edit_prize_money" name="prize_money" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_slots">Number of Slots</label>
                            <input type="number" class="form-control" id="edit_slots" name="slots" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_tournament_date">Tournament Date</label>
                            <input type="datetime-local" class="form-control" id="edit_tournament_date" name="tournament_date" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_registration_deadline">Registration Deadline</label>
                            <input type="datetime-local" class="form-control" id="edit_registration_deadline" name="registration_deadline" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_picture">Tournament Banner</label>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php
                        $tournaments_result = $conn->query("SELECT t.*, g.game_name, 
                        ...
                        <tr data-tournament-id="<?php echo $tournament['id']; ?>">
                            ...
                            <td id="game_type_<?php echo $tournament['id']; ?>"><?php echo htmlspecialchars($tournament['game_type']); ?></td>
                            ...
                            <td>
                                <div id="tournament_date_<?php echo $tournament['id']; ?>"><?php echo date('Y-m-d\TH:i', strtotime($tournament['tournament_date'])); ?></div>
                                <div id="registration_deadline_<?php echo $tournament['id']; ?>" style="display: none;"><?php echo date('Y-m-d\TH:i', strtotime($tournament['registration_deadline'])); ?></div>
                            </td>
                            <td>
                                <span class="badge badge-<?php 
                                    echo $tournament['status'] == 'upcoming' ? 'primary' : 
                            ...
                            <td>
                                ...
                                <?php if($tournament['status'] == 'upcoming' && $tournament['confirmed_teams'] == $tournament['slots']): ?>
                                <form method="POST" style="display: inline;">
                                    <input type="hidden" name="tournament_id" value="<?php echo $tournament['id']; ?>">
                                    <button type="submit" name="generate_fixtures" class="btn btn-success btn-sm btn-action" onclick="return confirm('Are you sure you want to generate fixtures for this tournament?')">
                                ...
                                <form method="POST" style="display: inline;">
                                    <input type="hidden" name="tournament_id" value="<?php echo $tournament['id']; ?>">
                                    <button type="submit" name="delete_tournament" class="btn btn-danger btn-sm btn-action" onclick="return confirm('Are you sure you want to delete this tournament? This will also delete all registrations and fixtures.')">
                                        <i class="fas fa-trash"></i> Delete
                                    </button>
                                </form>
                            </td>
